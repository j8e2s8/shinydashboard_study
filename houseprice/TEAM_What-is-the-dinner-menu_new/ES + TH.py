...
# 태현 추가. 시장 유형에 따른 스케일 비교
```{python}
# 그래프 그리기 위한 df 생성
# 시장 유형와 규모 크기에 따른 그룹화 / 규모 크기 갯수 추가
df_1 = market2.groupby(["type", "market_scale"], as_index = False) \
...
```{python}
# 규모 크기에 따른 내림차순
df_1.sort_values('market_count', ascending = False)

# 만든 df를 x축에 시장 유형, y축은 규모 크기에 따른 점포 갯수
plt.clf()
sns.barplot(data = df_1, y = 'type', x = 'market_count', hue = 'market_scale')
plt.show()
```
...
# 태현 추가, 한국 인구 그래프 그리기
```{python}
data_pop = subset.groupby('행정기관', as_index = False) \
                 .agg(n = ('총 인구수','min'))
data_pop = data_pop.sort_values('n', ascending = False)
data_pop
```
```{python}
import seaborn as sns
...
plt.clf()
sns.barplot(data = data_pop, y = '행정기관', x = 'n', hue = '행정기관')
plt.show()
```
```{python}
#서울과 경기도의 인구 비율 고려하기
(data_pop['n'][1] + data_pop['n'][8]) / 51529338 * 100
```
...
```{python}
#시장 점포 수 그래프
market_pop['over'] = np.where(market_pop['old_ratio'] >= 20.25, 'Y', 'N')

over_점포수 = market_pop.groupby(['region','over']) \
                        .agg(count = ('store_count','sum')) \
...
```{python}
plt.clf()
sns.barplot(data = over_점포수, y = 'region', x = 'count', hue = 'over', palette='Set1')
plt.show()
```


비교하기 편하게 2개 그래프 동시 보여주기
```{python}
plt.clf()
sns.barplot(data = data_pop, y = '행정기관', x = 'n', hue = '행정기관')
plt.show()
...
```{python}
# 지역별 노인 인구
서울_노인 = (subset.iloc[0,8:13].sum())
부산_노인 = (subset.iloc[1,8:13].sum())
대구_노인 = (subset.iloc[2,8:13].sum())
인천_노인 = (subset.iloc[3,8:13].sum())
광주_노인 = (subset.iloc[4,8:13].sum())
대전_노인 = (subset.iloc[5,8:13].sum())
울산_노인 = (subset.iloc[6,8:13].sum())
세종_노인 = (subset.iloc[7,8:13].sum())
경기_노인 = (subset.iloc[8,8:13].sum())
강원_노인 = (subset.iloc[9,8:13].sum())
충북_노인 = (subset.iloc[10,8:13].sum())
충남_노인 = (subset.iloc[11,8:13].sum())
전북_노인 = (subset.iloc[12,8:13].sum())
전남_노인 = (subset.iloc[13,8:13].sum())
경북_노인 = (subset.iloc[14,8:13].sum())
경남_노인 = (subset.iloc[15,8:13].sum())
제주_노인 = (subset.iloc[16,8:13].sum())
